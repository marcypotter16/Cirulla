...
class Tween:
    def __init__(self, target, duration, on_finish: callable = None, motion: str = "linear", **kwargs):
        self.motion = motion
        self.target = target
    ...
    def lerp(a, b, t):
        if isinstance(a, tuple) and isinstance(b, tuple):
            return tuple(Tween.lerp(a[i], b[i], t) for i in range(len(a)))
        return a + (b - a) * t
...
class TweenManager:
    ...
    def add_tween(self, target, tween_property, from_, to_, duration, on_finish: callable = None, motion: str = "linear"):
